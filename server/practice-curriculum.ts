...
import { getExercise, recommendation } from '../shared/catalog.js';
...
export function generatePracticeCurriculum(profile: Profile, previous: Assignment[]): Curriculum {
  ...
  return {
    ...
    summary: `${profile.role}의 데이터 처리 문제를 TypeScript 함수로 해결합니다. ${lessons.map(l => l.title).join(' → ')}. 목표: ${profile.goal}`,
    ...
    lessons: lessons.map(exercise => ({
      title: exercise.title, description: `${exercise.objective} ${profile.personaId === 'student' ? '전공 실험의 관측값을 코드의 입력과 연결해 보세요.' : '업무에서 잘못된 집계가 어떤 판단 오류로 이어지는지 생각해 보세요.'}`,
      objective: `${exercise.objective} ${guidance}`,
      prompt: `${exercise.contract}\n정답을 먼저 제시하지 말고 ${profile.major} / ${profile.role} 관점에서 필요한 개념과 작은 힌트를 설명해 줘.`,
      theory: exercise.theory,
      experiment: '합성 샘플로 실행하고 테스트의 기대값과 실제값을 비교하세요. 코드나 데이터를 바꾼 후에는 다시 실행해야 합니다. 입력이 없거나 일부 값이 누락되는 경우를 자신의 말로 설명하세요.',
      checks: ['브라우저에서 코드를 실행하고 테스트 결과를 확인했다','실패한 조건과 수정한 이유를 설명할 수 있다','테스트 통과가 모든 업무 데이터에 대한 정확성을 보장하지 않음을 이해했다'],
    })),
    quiz: [
