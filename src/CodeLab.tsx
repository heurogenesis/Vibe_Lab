...
export default function CodeLab({ exercise, assignmentId, profile, onSaved }: { exercise: Exercise; assignmentId?: string; profile?: Profile | null; onSaved?: (attempt: PracticeAttempt) => void }) {
  ...
  async function run() {
    ...
    try {
      ...
      } else if (exercise.language === 'r') {
        active.current = startRRun(exercise, code, rows);
      } else {
  ...
  return <section className="code-lab" aria-label="코드 실행 실습">
    <div className="section-title"><h3>코드 빈칸을 채우고 실행해 보세요</h3><span className="tag">{isSql ? 'SQL · SQLite 실행 테스트' : isR ? 'R · webR 실행 테스트' : 'TypeScript · 실행 테스트'}</span></div>
    ...
    <label className="code-label">{isSql ? 'SQL 질의문' : isR ? 'R 코드' : 'TypeScript 코드'}<textarea className="code-editor" aria-label={isSql ? 'SQL 실습 질의문' : isR ? 'R 실습 코드' : 'TypeScript 실습 코드'} spellCheck={false} maxLength={20000} value={code} disabled={saving} onChange={e=>edit(e.target.value)}/></label>
    ...
    <p className="small-note">{isSql ? '브라우저 안의 SQLite(WebAssembly)에서 실행합니다. 데이터는 메모리에만 올라가고 실행이 끝나면 사라집니다.' : isR ? '브라우저 안의 실제 R(webR)에서 실행합니다. 처음 실행할 때만 R 런타임 약 17MB를 내려받고, 이후에는 바로 실행됩니다.' : '문법 변환 후 브라우저에서 실행합니다. 전체 TypeScript 타입 검사는 제공하지 않습니다.'} 테스트는 고정된 검증 데이터, 샘플 출력은 위에서 선택한 데이터를 사용합니다.</p>
    ...
    <section className="prompt-kit">
      <strong><Sparkles size={15}/> LLM에게 물어볼 프롬프트 만들기<span className="ai-badge">AI</span></strong>
