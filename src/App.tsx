...
export default function App() {
  ...
  {loading ? <div className="empty-state" role="status"><LoaderCircle className="spin"/><h2>학습 공간을 불러오고 있어요.</h2></div> : !learner ? <Auth onAuthenticated={user=>void authenticated(user)}/> : !state ? <div className="empty-state"><h2>학습 공간에 연결하지 못했어요.</h2><button className="primary" onClick={()=>void load()}><RefreshCw size={16}/> 다시 연결</button></div> : <>
