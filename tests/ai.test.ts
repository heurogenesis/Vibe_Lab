...
it('sends structured output requirements and validates a completed Responses API curriculum',async()=> {
  const curriculum=generateRules(defaultProfile,[]);
  ...
  const result=await new LearningAI('test-api-key','test-model').generate(defaultProfile,[]);
...
it('surfaces AI failures without silently relabeling rule-based output as AI',async()=> {
  ...
  await expect(new LearningAI('test-api-key','test-model').generate(defaultProfile,[])).rejects.toMatchObject({status:502});
});
