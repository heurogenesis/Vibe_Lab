...
import { FileStore, PostgresStore } from '../server/store.js';
...
async function setup() {
  ...
  const store = new FileStore(join(folder,'state.json'));
  const github = new GitHubClient();
  const app = createApp(store,new LearningAI(),github);
  return {store,app,github};
}
describe('learner workflow via HTTP',()=> {
  it('persists profile → curriculum → progress → quiz → tutor, without leaking quiz answers',async()=> {
    ...
    await request(app).post('/api/assignments').set('X-Vibe-Lab','1').send({}).expect(400);
    await request(app).put('/api/profile').set('X-Vibe-Lab','1').send(defaultProfile).expect(200);
    const created = await request(app).post('/api/assignments').set('X-Vibe-Lab','1').send({}).expect(201);
    ...
    await request(app).patch(`/api/assignments/${a.id}/progress`).set('X-Vibe-Lab','1').send({step:0,completed:true}).expect(200);
    await request(app).patch(`/api/assignments/${a.id}/progress`).set('X-Vibe-Lab','1').send({step:0,completed:true}).expect(200);
    expect((await store.read()).assignments[0].completedSteps).toEqual([0]);
    await request(app).patch(`/api/assignments/${a.id}/progress`).set('X-Vibe-Lab','1').send({step:99,completed:true}).expect(400);
    await request(app).post(`/api/assignments/${a.id}/quiz`).set('X-Vibe-Lab','1').send({answers:[0,0]}).expect(400);
    const graded = await request(app).post(`/api/assignments/${a.id}/quiz`).set('X-Vibe-Lab','1').send({answers:[1,0,2]}).expect(200);
    expect(graded.body.score).toBe(3);
    const chat = await request(app).post(`/api/assignments/${a.id}/chat`).set('X-Vibe-Lab','1').send({message:'원리를 설명해 줘',lessonIndex:0}).expect(200);
    ...
    const reopened = new FileStore(join(folders[0],'state.json'));
    expect((await reopened.read()).profile?.major).toBe('경영학');
  });
  it('rejects invalid profiles, foreign origins and missing request headers',async()=> {
    ...
    await request(app).put('/api/profile').set('X-Vibe-Lab','1').send({...defaultProfile,minutes:-1}).expect(400);
    await request(app).put('/api/profile').set('X-Vibe-Lab','1').send({...defaultProfile,name:' '}).expect(400);
    await request(app).get('/api/no-such-route').expect(404);
    await request(app).post('/api/assignments/missing/chat').set('X-Vibe-Lab','1').send({message:'hello',lessonIndex:0}).expect(404);
  });
  it('serializes concurrent progress updates without losing either step',async()=> {
    ...
    await Promise.all([0,1,2].map(step=>request(app).patch(`/api/assignments/${id}/progress`).set('X-Vibe-Lab','1').send({step,completed:true}).expect(200)));
  ...
  it('saves submission evidence only after a valid repository check succeeds',async()=> {
    const {app,store,github}=await setup();
    ...
    await request(app).post(`/api/assignments/${id}/submission`).set('X-Vibe-Lab','1').send({url:'https://localhost/private',reflection}).expect(400);
    expect(evidence).not.toHaveBeenCalled();
    await request(app).post(`/api/assignments/${id}/submission`).set('X-Vibe-Lab','1').send({url:'https://github.com/example/course',reflection}).expect(200);
    ...
    await request(app).post(`/api/assignments/${id}/submission`).set('X-Vibe-Lab','1').send({url:'https://github.com/example/new-course',reflection}).expect(500);
